function [file_name ,seed_point_head]= rec_fragelo_CampoClaro_stopTerminalPoint_NOSEGMENTATION(folder_path, stack_name_prefix, timePoint, file_endpoints, seed_point,thresholdHead)
close all;
...
figure;set(gcf,'Visible', label_display_image); imshow(max(imageToDisplay,[],3)',[]);colormap('gray');hold on;
...
for step=1:1
    ...
    if exist('tracked_points.z_SP','var')
...
plot(tracked_points.x_SP{timePoint},tracked_points.y_SP{timePoint},'m.');
print(gcf,fullfile(folder_projection_output, [file_name '_rec.png']),'-dpng')
close all;
