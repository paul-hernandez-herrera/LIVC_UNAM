...
DEBUG=0;
    ...
    if (exist('relpativePath','var'))
            filename =  fullfile(relpativePath,filename );
    end   
    ...
    %Alberto
    if java.io.File([filename '.mhd' ]).exists()
        % Add the extension
        filename = [filename , '.mhd'  ];
    elseif java.io.File(filename).exists() && ~exist(filename,'dir')
    ...
    if (exist('relpativePath','var'))
        ...
        dataFile =  fullfile(relpativePath,dataFile );
    end      
